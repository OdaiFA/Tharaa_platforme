...
@section('content')
    ...
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        @forelse ($accounts as $account)
            <div class="bg-financial-card relative overflow-hidden p-5 text-white">
                ...
                <div class="relative mt-4 flex items-center justify-end gap-2 border-t border-white/10 pt-3 text-xs">
                    ...
                    <form method="POST" action="{{ route('accounts.destroy', $account) }}" onsubmit="return confirm('هل أنت متأكد من حذف هذا الحساب؟')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="rounded-lg bg-red-500/20 px-3 py-1.5 font-medium text-red-100 hover:bg-red-500/40">حذف</button>
                    </form>
                </div>
            </div>
        @empty
