<div>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex gap-2">
            @if ($confirmingDelete)
                <span class="flex items-center text-sm font-medium text-red-600">حذف هذه الميزانية؟</span>
                <button type="button" wire:click="delete" wire:loading.attr="disabled" wire:target="delete"
                    class="rounded-xl bg-red-600 px-4 py-2 text-sm font-bold text-white hover:bg-red-700 disabled:opacity-60">نعم، احذف</button>
                <button type="button" wire:click="cancelDelete" class="rounded-xl border border-gray-300 bg-white px-4 py-2 text-sm font-bold text-gray-700 hover:bg-gray-50">إلغاء</button>
            @else
                ...
                <button type="button" wire:click="confirmDelete" class="rounded-xl bg-red-50 px-4 py-2 text-sm font-bold text-red-600 hover:bg-red-100">حذف</button>
            @endif
        </div>
    </div>
