        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($articles as $article)
                    <tr wire:key="article-{{ $article->id }}" class="hover:bg-gray-50/50">
                        ...
                        <td class="px-4 py-3">
                            @if ($article->trashed())
                                <span class="rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-medium text-red-600">محذوف</span>
                            @else
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            @if ($article->trashed())
                                <button type="button" wire:click="restore({{ $article->id }})" class="text-green-600 hover:underline">استعادة</button>
                            @else
                                <a href="{{ route('admin.articles.edit', $article) }}" class="text-amber-600 hover:underline">تعديل</a>
                                @if ($confirmingDeleteId === $article->id)
                                    <button type="button" wire:click="delete({{ $article->id }})" class="mr-2 text-red-600 hover:underline">تأكيد الحذف</button>
                                    <button type="button" wire:click="cancelDelete" class="mr-2 text-gray-500 hover:underline">إلغاء</button>
                                @else
                                    <button type="button" wire:click="confirmDelete({{ $article->id }})" class="mr-2 text-red-600 hover:underline">حذف</button>
                                @endif
                            @endif
                        </td>
                    </tr>
