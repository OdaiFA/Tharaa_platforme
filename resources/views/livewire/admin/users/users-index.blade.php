<div>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-2">
            <input type="text" wire:model.live.debounce.400ms="search" placeholder="بحث بالاسم أو البريد..."
                class="rounded-xl border border-gray-300 px-4 py-2 text-sm focus:border-primary-500 focus:outline-none">
            <span wire:loading wire:target="search" class="text-xs text-gray-400">جارٍ البحث...</span>
        </div>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse ($users as $user)
                    <tr wire:key="user-{{ $user->id }}" class="hover:bg-gray-50/50">
                        <td class="px-4 py-3">
