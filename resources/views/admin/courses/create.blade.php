...
@section('content')
    <div class="mx-auto max-w-2xl">
        <h1 class="text-2xl font-extrabold text-gray-900">إنشاء دورة جديدة</h1>

        <form method="POST" action="{{ route('admin.courses.store') }}" enctype="multipart/form-data" class="mt-6 space-y-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            @csrf
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="duration_hours" type="number" min="0" name="duration_hours" value="{{ old('duration_hours', 2) }}"
                        class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20">
                </div>
                <div>
                    <label for="passing_score" class="mb-1.5 block text-sm font-medium text-gray-700">درجة النجاح (%)</label>
                    <input id="passing_score" type="number" min="0" max="100" name="passing_score" value="{{ old('passing_score', 60) }}"
                        class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20">
                </div>
                <div>
                    ...
                    <select id="age_groups" name="age_groups[]" multiple
                        ...
                        @foreach ($ageGroups as $ageGroup)
                            <option value="{{ $ageGroup->id }}" @selected(in_array($ageGroup->id, old('age_groups', [])))>{{ $ageGroup->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
            ...
            <div class="space-y-2">
                <label class="flex items-center gap-2 text-sm text-gray-700">
                    <input type="checkbox" name="is_published" value="1" @checked(old('is_published')) class="h-4 w-4 text-primary-600"> نشر الدورة
                </label>
                <label class="flex items-center gap-2 text-sm text-gray-700">
                    <input type="checkbox" name="certificate_enabled" value="1" @checked(old('certificate_enabled', true)) class="h-4 w-4 text-primary-600"> تفعيل شهادة الإتمام
                </label>
            </div>

            <button type="submit" class="w-full rounded-xl bg-primary-600 py-3 font-bold text-white hover:bg-primary-700">إنشاء الدورة</button>
        </form>
    </div>
@endsection
