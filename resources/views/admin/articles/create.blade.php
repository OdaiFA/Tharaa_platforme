...
@section('content')
    <div class="mx-auto max-w-2xl">
        <h1 class="text-2xl font-extrabold text-gray-900">إنشاء مقال جديد</h1>

        <form method="POST" action="{{ route('admin.articles.store') }}" enctype="multipart/form-data" class="mt-6 space-y-4 rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            @csrf
            ...
            <div>
                ...
                <textarea id="excerpt" name="excerpt" rows="2" maxlength="500"
                    class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20">{{ old('excerpt') }}</textarea>
            </div>

            <div>
                <label for="content" class="mb-1.5 block text-sm font-medium text-gray-700">المحتوى (يدعم Markdown)</label>
                <textarea id="content" name="content" rows="10" required
                    class="w-full rounded-xl border border-gray-300 px-4 py-2.5 text-sm focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500/20">{{ old('content') }}</textarea>
            </div>
            ...
            <label class="flex items-center gap-2 text-sm text-gray-700">
                <input type="checkbox" name="is_published" value="1" @checked(old('is_published')) class="h-4 w-4 text-primary-600"> نشر المقال
            </label>

            <button type="submit" class="w-full rounded-xl bg-primary-600 py-3 font-bold text-white hover:bg-primary-700">إنشاء المقال</button>
        </form>
    </div>
@endsection
