...
@section('content')
    ...
    @if ($categories->isNotEmpty())
        <div class="mb-6 flex flex-wrap gap-2">
            <a href="{{ route('articles.index') }}"
                class="rounded-full px-4 py-1.5 text-sm font-medium {{ ! request('category_id') ? 'bg-primary-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100' }}">
                الكل
            </a>
            @foreach ($categories as $category)
                <a href="{{ route('articles.index', ['category_id' => $category->id]) }}"
                    class="rounded-full px-4 py-1.5 text-sm font-medium {{ request('category_id') == $category->id ? 'bg-primary-600 text-white' : 'bg-white text-gray-600 hover:bg-gray-100' }}">
    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        @forelse ($articles as $article)
            <a href="{{ route('articles.show', $article) }}" class="group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition hover:shadow-md">
