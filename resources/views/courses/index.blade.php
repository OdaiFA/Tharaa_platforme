...
@section('content')
    ...
    <form method="GET" class="mb-6 grid grid-cols-1 gap-3 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm sm:grid-cols-3">
        <div>
            <label class="mb-1 block text-xs font-medium text-gray-500">بحث</label>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="ابحث عن دورة..."
        ...
        <div>
            ...
            <select name="level" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none">
                <option value="">الكل</option>
                <option value="beginner" @selected(request('level') === 'beginner')>مبتدئ</option>
                <option value="intermediate" @selected(request('level') === 'intermediate')>متوسط</option>
                <option value="advanced" @selected(request('level') === 'advanced')>متقدم</option>
            </select>
        </div>
        <div>
            <label class="mb-1 block text-xs font-medium text-gray-500">الفئة العمرية</label>
            <select name="age_group_id" class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-primary-500 focus:outline-none">
                <option value="">الكل</option>
                @foreach (\App\Models\AgeGroup::orderBy('min_age')->get() as $ageGroup)
                    <option value="{{ $ageGroup->id }}" @selected(request('age_group_id') == $ageGroup->id)>{{ $ageGroup->name }}</option>
                @endforeach
            </select>
        </div>
    </form>
    ...
    <div class="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
        @forelse ($courses as $course)
            <a href="{{ route('courses.show', $course) }}" class="group overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-sm transition hover:shadow-md">
